...
type BusAssistantDashboardStats = {
...
const BusAssistantDashboardMenuItem = () => {
    const { user } = useAuth()
    const [profile, setProfile] = useState<AuthenticatedUserRecord | null>(null)
    const [stats, setStats] = useState<BusAssistantDashboardStats>({
        incidentCount: 0,
        complaintCount: 0,
        totalReports: 0,
        weeklyReports: 0,
    ...
    const loadDashboard = async (withLoader = true) => {
        ...
        if (failedRequests.length === results.length) {
            setErrorMessage('Failed to load the bus assistant dashboard.')
    ...
    if (isLoading) {
        return <Loader variant="section" label="Loading bus assistant dashboard" />
    }
                ...
                <section className="driverMenuDashboard__panel">
                    ...
                    {recentActivity.length === 0 ? (
                        <p className="driverMenuDashboard__emptyText">
                            No bus assistant activity has been recorded yet.
                        </p>
                    ) : (
...
export default BusAssistantDashboardMenuItem
